<template>
  <div :class="wrapperClass">
    <div class="flex flex-wrap justify-center">
      <div class="m-2 w-[320px] rounded-lg" v-for="(phase, key) in phases" :style="'border: 3px solid ' + phase.color[1]">

        <div class="phaseHead rounded-t-md" :style="'background-color: ' + phase.color[0] +'; border-bottom: 3px solid ' + phase.color[1] + ';'">
        ...
        <ul class="py-2 px-2">
          <li v-for="lesson in phase.lessons">
            <button @click="showLinks(lesson, phase)" @mouseenter="getRelatedLessons(lesson.id)" @mouseleave="allRelatedLessons = []" :class="allRelatedLessons.includes(lesson.id) ? phase.title+'linked' : phase.title+'butt'" class="w-full rounded px-2 pb-2 pt-2.5 font-medium text-lg btn-roadmap">
              {{ lesson.title }}
            </button>
          </li>
        </ul>
